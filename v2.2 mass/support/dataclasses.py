...
class Vector2:
    ...
    def norm_v_res(self,d):
        """obtain vector of normal res"""
        return self-self.v_res(d)
    ...
    def reflect_v(self,d,ratio=1):
        """
        for a given vector self and a direction vector d, it will reflect vector component of v paralel to d
        
        :param v: original vector
        :type v: Vector2
        :param d: direction vector (does not have to be unit)
        :type d: Vector2
        :param ratio: how much will the paralel vector component not be lost? [0to1]

        does not mod original, returns new
        """
        ...
        d=d.unit_v()
        vc=self.v_res(d)
        return self - vc*(1+ratio)
    
    def scale_to_mag(self,v):
        n=self.copy()
        return n.unit_v() * v
