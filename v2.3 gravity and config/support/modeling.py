...
class Ball:
    __slots__=["pos","v","a","r","mass","restitution","oldpos","id","potential_phasing","debug_info","fixed","trail_cl","t_since_last_trailcheck"]
    def __init__(self,pos,v=(0,0),a=(0,0),
                 r:int=3,mass:int=1,
                 restitution:float=1,fixed:bool=False):
        ...
        self.t_since_last_trailcheck=time.time()
        self.trail_cl:TrailQueue=None
...
class Physics:
    ...
    def apply_sva_kinematics(self,ball:Ball,dt):
        ...
        if not ball.fixed:
            ...
            ball.a,ball.v=Vector2(0,0),Vector2(0,0)
        ...
        if not ball.potential_phasing:
            ...
            for ball2 in bl:
                ...
                if dist_sqr<=r_total**2:
                    ...
                    if ball.fixed:
                        ball2.v=ball2.v.reflect_v(d_pv)
                        ball2.pos=op2
                    if ball2.fixed:
                        ball.v=ball.v.reflect_v(d_pv)
                        ball.pos=op1
                    else:
    ...
    def apply_all_gravity(self):
        if self.world.config.gameConfig.g_coef!=0:
            for i1,ball1 in enumerate(self.world.balls):
                ...
                for i2,ball2 in enumerate(self.world.balls):    
                    if i1!=i2:
                        ...
                        dist_sqr=v_toBall2.mag_sqr()
                        if dist_sqr > (ball1.r + ball2.r)**2:

                            strengh=self.world.g_coef * (ball1.mass * ball2.mass) / (dist_sqr) / ball1.mass
    ...
    def tstep_physics(self,dt):
        ...
        if self.world.config.gameConfig.do_wall_ball_collisions:
            for i ,ball in enumerate(bl):
                self.apply_all_wall_collosions(ball)
        
        if self.world.config.gameConfig.do_ball_ball_collisions:
            for i ,ball in enumerate(bl):
                self.apply_all_ball_collosions(ball)

        if self.world.config.gameConfig.do_gravity:
            for i ,ball in enumerate(bl):
        ...
        if self.world.gameobj.show_ball_id:
            ...
            for ball in bl:
                ...
                ball.debug_info=str(round(ball.v.magnitude()))

        if self.world.gameobj.toggle_ball_trails:
            for ball in bl:
                ct=time.time()
                dt=ct-ball.t_since_last_trailcheck
                if dt > 1/50:
                    ball.trail_cl.append(ball.pos)
                    ball.t_since_last_trailcheck=ct
...
class World:
    ...
    def __init__(self,config:Config,gameobj=None):
        """
        ...
        """
        self.config=config
        self.dim=config.windowConfig.gdim.as_tup()
        ...
        self.max_ball_radius=config.gameConfig.max_ball_size
        ...
        self.g_coef=config.gameConfig.g_coef
    ...
    def add_ball(self,ball:Ball=None):
        ...
        if self.gameobj.toggle_ball_trails:
            ball.trail_cl=TrailQueue(self.config.windowConfig.ball_trail_list_len)
    ...
    def trailtoggle_toggleTrailQueueADT(self):
        if self.gameobj.render_all_ball_trails:
            ct=time.time()
    ...
    def add_orbiter(self,main:Ball,orbit_r:int,clockwise=False,mass=1,ball_r=3):
        offset:Vector2=Vector2.random()
        offset=offset.unit_v()*orbit_r

        ball2pos=main.pos+offset

        
        normal=Vector2(-offset.y,offset.x).unit_v()
        if not clockwise:
            normal*=-1

        
        v_init=math.sqrt(self.config.gameConfig.g_coef * main.mass / orbit_r)

        self.add_ball(Ball(ball2pos,normal.unit_v()*v_init, mass=mass, r=ball_r))
